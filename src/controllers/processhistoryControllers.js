...
exports.validationOnCreate = async (req, res, next) => {
    try{
        ...
        if (result.recordset.length >0 )throw new Error(`Duplicate records found for budgetId ${budgetId}`)
...
exports.validationOnUdpdate = async (req, res, next) => {
    try{
        const {budgetId, reviewLevel, approvedBy} = req.body
        ...
        if (result.recordset.length > 0)throw new Error(`Duplicate records found for budget id`)
