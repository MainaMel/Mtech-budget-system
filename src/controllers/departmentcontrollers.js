...
exports.validationOnCreate = async (req, res, next) => {
    ...
    try {
        ...
        const query = ` select * from departments where department = '${department}' and hod = ${hod}  `
        ...
        //  if(result. recordset.length > 0) throw new Error(`department <${department}> already exists`)
         ...
         if(result.recordset.length > 0) {
            return res.status(400).json({ error: `Department <${department}> already exists` });
        }
         next()
...
exports.validationOnUpdate = async (req, res, next) => {
 ...
 try{
    ...
    const result = await pool
    .request()
    .input('d',sql.Int,req.params.id)
    .query(query)

    if (result.recordset.length === 0) throw new Error('Department not found');
